...
* **Authentication:** JWT (JSON Web Tokens), Role-based Authorization (Admin, Tutor, Student)
...
4. Mở file `appsettings.json` tại thư mục API và cấu hình `DefaultConnection` trỏ tới database SQL Server của bạn.
5. Nhấn `F5` hoặc chạy lệnh `dotnet run` để khởi động Server và Swagger UI.
